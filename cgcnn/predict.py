...
args = argparse.Namespace(modelpath='C:/Users/hzz6536/Documents/Python/cgcnn-master/pre-trained/formation-energy-per-atom.pth.tar',
                          cifpath='D:/PSED/jarvis_cifs',
                          disable_cuda=False,
                          batch_size=256,
                          print_freq=10,
                          workers=0)
...
def validate(val_loader, model, criterion, normalizer, test=False):
    ...
    if test:
        ...
        with open('test_results.csv', 'w') as f:
            writer = csv.writer(f)
            for cif_id, target, pred, feature in zip(test_cif_ids, test_targets,
                                            test_preds, test_features):
                writer.writerow((cif_id, target, pred, feature))
    else:
        star_label = '*'

    '''output, added'''
    output = pd.DataFrame({'cif_id':test_cif_ids, 'target':test_targets, 'pred':test_preds, 'feature':test_features})
    output.to_pickle('cgcnn_features.pkl')
